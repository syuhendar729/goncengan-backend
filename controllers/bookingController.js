...
const driverCreateRoom = async (req, res) => {
    try {
        ...
        await bookingRoomDoc.set({
            bookingId, chatRoomId: null,
			departureDate: Timestamp.fromDate(new Date(req.body.departureDate)),
			distance: null, driver: { ...resDriver, departure, destination },
			isArrive: false, isBooked: false, isDepart: false,
            isPayed: false, isPicked: false, passenger: null,
            paymentId: null, price: null,
...
const driverCancelRoom = async (req, res) => {
    try {
        ...
        res.send({ message: 'Successfully cancel dan delete BookingRoom from driver!', data: { bookingId, driverId: req.uid } })
    } catch (error) {
        console.log(error)
...
const passengerCancelRoom = async (req, res) => {
    try {
		...
		if (passenger === null) res.status(400).send({ message: "Passenger is null, can't cancel BookingRoom!" })
            ...
            await bookingRoomDoc.update({ 
				distance: null, isBooked: false,
				passenger: null, price: null,
